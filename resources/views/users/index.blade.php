...
<div class="users-header">
  ...
  <div class="d-flex align-items-center gap-2" style="flex-wrap:wrap;">
    ...
    <a href="{{ route('users.create') }}" class="btn btn-sm btn-add">+ Nuevo</a>
  </div>
</div>
    ...
    <tbody>
      @forelse($users as $u)
        <tr>
          ...
          <td>{{ $u->email }}</td>
          <td class="text-end">
            <a href="{{ route('users.edit', $u) }}" class="btn btn-sm btn-outline-secondary">✏️ Editar</a>

            <form action="{{ route('users.destroy', $u) }}" method="POST" style="display:inline-block;" onsubmit="return confirmDelete(event, this, '{{ $u->name }}')">
              @csrf
              @method('DELETE')
        ...
        <tr>
          <td colspan="3">
            <div class="empty-state">
              ...
              <a href="{{ route('users.create') }}" class="btn btn-sm btn-add">➕ Crear usuario</a>
            </div>
          </td>
        </tr>
      @endforelse
