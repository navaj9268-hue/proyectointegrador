...
<div class="container">
  <div class="d-flex justify-content-between align-items-center mb-3">
    ...
    <div>
      <a href="{{ route('reservations.calendar') }}" class="btn btn-sm btn-outline-secondary">← Calendario</a>
      <a href="{{ route('reservations.index') }}" class="btn btn-sm btn-outline-secondary">Lista</a>
    </div>
  </div>
  ...
  <div class="row">
    <div class="col-md-8">
      <div class="card mb-3 card-accent p-3">
        ...
        <div class="mt-3">
          <a href="{{ route('reservations.edit', $reservation) ?? '#' }}" class="btn btn-sm btn-outline-secondary">✏️ Editar</a>

          <form action="{{ route('reservations.destroy', $reservation) }}" method="POST" style="display:inline-block;" onsubmit="return confirm('Eliminar reserva?')">
            @csrf
            @method('DELETE')
