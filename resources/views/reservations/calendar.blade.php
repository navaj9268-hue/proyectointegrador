      ...
      <div class="modal-body">
        ...
        <div class="row g-2 mb-2">
          <div class="col">
            ...
            <input type="date" name="checkin_at" id="checkin_at" class="form-control" required>
          </div>
          <div class="col">
            <label class="form-label">Check-out</label>
            <input type="date" name="checkout_at" id="checkout_at" class="form-control" required>
          </div>
        </div>
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    const calendar = new FullCalendar.Calendar(calendarEl, {
      ...
      eventDrop: function(info) {
        ...
        fetch(`{{ url('reservations') }}/${id}`, {
          ...
          body: JSON.stringify({ checkin_at: start, checkout_at: end })
    ...
    form.addEventListener('submit', function(e){
      ...
      const payload = {
        ...
        checkin_at: document.getElementById('checkin_at').value,
        checkout_at: document.getElementById('checkout_at').value,
        notes: document.getElementById('notes').value,
      };

      if (!payload.guest_name || !payload.checkin_at || !payload.checkout_at) {
        alert('Rellena huésped y fechas.');
        return;
      }

      const url = id ? `{{ url('reservations') }}/${id}` : '{{ route("reservations.store") }}';
