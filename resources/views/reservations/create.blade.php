...
<div class="container">
  <div class="d-flex justify-content-between align-items-center mb-3">
    ...
    <div>
      <a href="{{ route('reservations.calendar') }}" class="btn btn-outline-secondary">← Volver al calendario</a>
      <a href="{{ route('reservations.create') }}" class="btn btn-sm btn-light ms-2">Nueva</a>
    </div>
  </div>
      ...
      <form id="reserveForm" method="POST" action="{{ route('reservations.store') }}">
        ...
        <div class="row g-3">
          <div class="col-md-4">
            ...
            <input id="checkin" name="checkin_at" value="{{ old('checkin_at') }}" type="date" class="form-control" required>
          </div>

          <div class="col-md-4">
            <label class="form-label">Check-out</label>
            <input id="checkout" name="checkout_at" value="{{ old('checkout_at') }}" type="date" class="form-control" required>
          </div>
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  async function buscar() {
    ...
    try {
      const url = `{{ route('reservations.available') }}?checkin=${encodeURIComponent(cIn)}&checkout=${encodeURIComponent(cOut)}`;
