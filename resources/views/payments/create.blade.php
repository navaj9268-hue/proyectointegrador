  ...
  <form method="POST" action="{{ isset($payment) ? route('payments.update', $payment) : route('payments.store') }}">
    ...
    <div class="row g-2">
      ...
      <div class="col-md-6 mb-2">
        ...
        <input name="amount" type="number" step="0.01" value="{{ old('amount', $payment->amount ?? '') }}" class="form-control" required>
      </div>

      <div class="col-md-4 mb-2">
        <label class="form-label">Método</label>
        <select name="method" class="form-select" required>
          <option value="efectivo" @selected(old('method', $payment->method ?? '') == 'efectivo')>Efectivo</option>
          <option value="tarjeta" @selected(old('method', $payment->method ?? '') == 'tarjeta')>Tarjeta</option>
          <option value="transferencia" @selected(old('method', $payment->method ?? '') == 'transferencia')>Transferencia</option>
          <option value="otro" @selected(old('method', $payment->method ?? '') == 'otro')>Otro</option>
        </select>
      </div>

      <div class="col-md-4 mb-2">
        <label class="form-label">ID Transacción</label>
        <input name="transaction_id" value="{{ old('transaction_id', $payment->transaction_id ?? '') }}" class="form-control">
      </div>

      <div class="col-md-4 mb-2">
        <label class="form-label">Pagó (nombre)</label>
        <input name="payer_name" value="{{ old('payer_name', $payment->payer_name ?? '') }}" class="form-control">
      </div>
