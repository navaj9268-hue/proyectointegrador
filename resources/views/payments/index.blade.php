...
<div class="d-flex justify-content-between align-items-center mb-3">
  <h4>Pagos</h4>
  <a href="{{ route('payments.create') }}" class="btn btn-sm" style="background:linear-gradient(90deg,#b23a3a,#ff6b6b); color:#fff;">+ Nuevo pago</a>
</div>
      ...
      <tbody>
        @forelse($payments as $p)
          <tr>
            ...
            <td>
              @if($p->reservation)
                <a href="{{ route('reservations.show', $p->reservation) }}">{{ $p->reservation->guest->name ?? '-' }}</a><br>
            ...
            <td>{{ $p->user->name ?? '-' }}</td>
            <td class="text-end">
              <a class="btn btn-sm btn-outline-secondary" href="{{ route('payments.show', $p) }}">Ver</a>
              <a class="btn btn-sm btn-outline-primary" href="{{ route('payments.edit', $p) }}">Editar</a>
              <form action="{{ route('payments.destroy', $p) }}" method="POST" style="display:inline-block;" onsubmit="return confirm('Eliminar pago?')">
                @csrf @method('DELETE')
